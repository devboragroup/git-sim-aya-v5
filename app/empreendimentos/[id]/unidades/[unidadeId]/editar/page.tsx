...
export async function generateMetadata({ params }: EditarUnidadePageProps): Promise<Metadata> {
  ...
  return {
    title: `Editar ${unidade.identificador} - ${empreendimento?.nome || "Empreendimento"} - Simulador AYA`,
    description: `Editar informações da unidade ${unidade.identificador}`,
  }
}
    ...
    <DashboardShell>
      <div className="flex items-center justify-between">
        <Button variant="outline" className="border-gray-700 text-gray-300 hover:bg-gray-800" asChild>
          <Link href={`/empreendimentos/${params.id}/unidades/${params.unidadeId}`}>
            <ArrowLeft className="mr-2 h-4 w-4" />
            Voltar para Detalhes da Unidade
          </Link>
        </Button>
      </div>

      <DashboardHeader
        heading={`Editar ${unidade.identificador}`}
        text={`Editar informações da unidade no empreendimento ${empreendimento?.nome || ""}`}
      />

      <div className="grid gap-6">
        <UnidadeForm empreendimentoId={params.id} unidade={unidade} />
      </div>
    </DashboardShell>
  )
}
